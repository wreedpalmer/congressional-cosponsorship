...
def getData(congress, getBillCosponsors=True, getAmendmentData=True):
    ...
    while billsPull['num_results'] > 0:
        ...
        for billDict in billsList:
            ...
            if billDict['cosponsors'] > 0 and getBillCosponsors:
                cosponsors.extend(getCosponsorship(congress, billDict['bill_slug']))

            if getAmendmentData:
                amendments, amendmentCosponsors = pullAmendmentData(billDict['bill_slug'], congress)
                billDictToAppend['amendments'] = len(amendments)
                billDictToAppend['amendment_cosponsors'] = len(amendmentCosponsors)
                bills.append(billDictToAppend)
...
#for congress in range(113,117):
#    getData(congress)

getData(114, True, True)